...
export default function App() {
	...
	const handleSerialNumberChange = (e, index) => {
		...
		setForm((prevForm) => {
			const newSerialNumbers = [...prevForm.serialNumber];
	...
	const processSerialNumber = async (index, input) => {
		...
		// Update quantity
		newQuantities[index] = qtyPart ? Number(qtyPart) : 1;
		...
		// Fetch item name from Supabase based on modelCode
		...
		try {
			...
			if (error) {
                ...
                console.log("form.item.length: ",form.item.length);
                if (form.item.length === index + 1) {
                    handleAddSerialQ();
                }
			}
		} catch (err) {
			console.error(`Unexpected error for index ${index}:`, err);
		}
	};
	...
	const handleAddSerialQ = () => {
		setForm((prevForm) => ({
			...prevForm,
			serialNumber: [...prevForm.serialNumber, ""],
	...
	const backupPDF = () => {
		...
		const customerPart = backupCustomer || "all";
		doc.save(
			`inventory_backup_${customerPart}_${selectedEntries.length}_rows.pdf`
		);
	};
